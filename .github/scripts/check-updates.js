...
async function getLatestProviderVersion(namespace, name) {
  try {
    ...
    if (!response.ok) {
      console.error(`Failed to fetch versions for ${namespace}/${name}: ${response.status}`);
      return null;
    }
...
async function getProviderChangelog(namespace, name, version) {
  try {
    ...
    console.error(`Error fetching changelog for ${namespace}/${name}@${version}:`, error);
    return null;
  }
}
...
function updatePackage(packagePath, currentProvider, newVersion, changelog) {
  ...
  const encodedParam = Buffer.from(JSON.stringify(newParamValue)).toString('base64');
...
async function main() {
  ...
  for (const pkg of packages) {
    ...
    const decodedParam = Buffer.from(packageJson.pulumi.parameterization.value, 'base64').toString('utf8');
  ...
  if (updates.length > 0) {
    ...
    // Create release plan
    const releaseMessage = updates.map(u => 
      `Update ${u.name} from ${u.oldVersion} to ${u.newVersion}`
    ).join('\n\n');
