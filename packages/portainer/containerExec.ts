// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "./utilities";

export class ContainerExec extends pulumi.CustomResource {
    /**
     * Get an existing ContainerExec resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: ContainerExecState, opts?: pulumi.CustomResourceOptions): ContainerExec {
        return new ContainerExec(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'portainer:index/containerExec:ContainerExec';

    /**
     * Returns true if the given object is an instance of ContainerExec.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is ContainerExec {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === ContainerExec.__pulumiType;
    }

    declare public readonly command: pulumi.Output<string>;
    declare public readonly containerExecId: pulumi.Output<string>;
    declare public readonly endpointId: pulumi.Output<number>;
    /**
     * Deployment mode: 'standalone' (default) or 'swarm'
     */
    declare public readonly mode: pulumi.Output<string | undefined>;
    declare public /*out*/ readonly output: pulumi.Output<string>;
    declare public readonly serviceName: pulumi.Output<string>;
    declare public readonly user: pulumi.Output<string | undefined>;
    declare public readonly wait: pulumi.Output<number | undefined>;

    /**
     * Create a ContainerExec resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: ContainerExecArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: ContainerExecArgs | ContainerExecState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as ContainerExecState | undefined;
            resourceInputs["command"] = state?.command;
            resourceInputs["containerExecId"] = state?.containerExecId;
            resourceInputs["endpointId"] = state?.endpointId;
            resourceInputs["mode"] = state?.mode;
            resourceInputs["output"] = state?.output;
            resourceInputs["serviceName"] = state?.serviceName;
            resourceInputs["user"] = state?.user;
            resourceInputs["wait"] = state?.wait;
        } else {
            const args = argsOrState as ContainerExecArgs | undefined;
            if (args?.command === undefined && !opts.urn) {
                throw new Error("Missing required property 'command'");
            }
            if (args?.endpointId === undefined && !opts.urn) {
                throw new Error("Missing required property 'endpointId'");
            }
            if (args?.serviceName === undefined && !opts.urn) {
                throw new Error("Missing required property 'serviceName'");
            }
            resourceInputs["command"] = args?.command;
            resourceInputs["containerExecId"] = args?.containerExecId;
            resourceInputs["endpointId"] = args?.endpointId;
            resourceInputs["mode"] = args?.mode;
            resourceInputs["serviceName"] = args?.serviceName;
            resourceInputs["user"] = args?.user;
            resourceInputs["wait"] = args?.wait;
            resourceInputs["output"] = undefined /*out*/;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(ContainerExec.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}

/**
 * Input properties used for looking up and filtering ContainerExec resources.
 */
export interface ContainerExecState {
    command?: pulumi.Input<string>;
    containerExecId?: pulumi.Input<string>;
    endpointId?: pulumi.Input<number>;
    /**
     * Deployment mode: 'standalone' (default) or 'swarm'
     */
    mode?: pulumi.Input<string>;
    output?: pulumi.Input<string>;
    serviceName?: pulumi.Input<string>;
    user?: pulumi.Input<string>;
    wait?: pulumi.Input<number>;
}

/**
 * The set of arguments for constructing a ContainerExec resource.
 */
export interface ContainerExecArgs {
    command: pulumi.Input<string>;
    containerExecId?: pulumi.Input<string>;
    endpointId: pulumi.Input<number>;
    /**
     * Deployment mode: 'standalone' (default) or 'swarm'
     */
    mode?: pulumi.Input<string>;
    serviceName: pulumi.Input<string>;
    user?: pulumi.Input<string>;
    wait?: pulumi.Input<number>;
}
