...
# Run all checks (build + typecheck + test:scripts + syncpack + oxfmt + actionlint + actions-lock + zizmor)
...
# GitHub Actions lint / security audit / lockfile verify
pnpm nx run root:actionlint
pnpm nx run root:actions-lock
...
- Workflows are managed by [`gh actions-lock`](https://github.com/github/gh-actions-lock): tag refs in YAML, immutable SHAs in `.github/workflows/actions.lock`. After adding/changing `uses:` (or bumping action tags), run `gh actions-lock` and commit the lockfile. CI validates with `gh actions-lock --verify` (`pnpm nx run root:actions-lock`).
- In-repo composite actions use the self-repository form `$/.github/actions/...` (not `./...`).
- Top-level `permissions: contents: read`; jobs escalate as needed.
- Node, pnpm, the store cache, and `pnpm install` come from `pnpm/setup` (v11+; not `pnpm/action-setup`). Renovate regex managers track `version` / `runtime: node@` and group those bumps with `mise.toml`.
- Renovate regenerates `actions.lock` after github-actions updates (since Renovate 44.24); pin the CLI extension via `constraints.ghActionsLock`. Keep `pinDigests: false` for that manager so PRs stay on tag refs.
...
- Before claiming a build works, run `pnpm nx affected -t check` -- it covers typecheck, build, oxfmt, syncpack, actionlint, actions-lock, and zizmor.
