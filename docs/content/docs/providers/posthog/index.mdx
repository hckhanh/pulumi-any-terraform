...
const signupAlert = new posthog.Alert("signup-alert", {
    ...
    insight: 12345, // Use the numeric ID from PostHog
    enabled: true,
    thresholdType: "absolute",
    thresholdLower: 10,
    subscribedUsers: [67890], // User IDs to notify
...
- `filters` (string): JSON string containing targeting and rollout configuration
- `rolloutPercentage` (number): Overall rollout percentage (0-100)
