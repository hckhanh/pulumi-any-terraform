...
- **GitHub Actions Cache**: pnpm store and the Nx task cache (`.nx`) persisted across runs
...
Nx caches task outputs locally and skips tasks whose inputs are unchanged. CI persists the whole `.nx` directory (cache + its db) across runs via `actions/cache`, so unchanged tasks are restored instead of rebuilt. There is no remote cache _service_ (Nx Cloud / S3); correctness always comes from Nx's input-hashing, and a cache produced on a different runner is safely ignored rather than trusted.
...
Caches the pnpm store and the Nx task cache (`.nx`) across runs. Both keys include the `pnpm-lock.yaml` hash so dependency changes never reuse a stale Nx cache:
...
- uses: actions/cache@v6
  with:
    path: .nx
    key: ${{ runner.os }}-nx-${{ hashFiles('**/pnpm-lock.yaml') }}-${{ github.run_id }}
    restore-keys: |
