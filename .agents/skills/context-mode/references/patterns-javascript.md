...
console.log(
  `Paths aliases: ${Object.keys(data.compilerOptions?.paths || {}).length}`,
)
...
try {
  ...
  lockExists.yarn = true
} catch {}
try {
  fs.accessSync('pnpm-lock.yaml')
  lockExists.pnpm = true
} catch {}
...
let output
try {
  output = execSync('npx jest --json 2>/dev/null', {
    encoding: 'utf8',
...
try {
  ...
  console.log(
    `Suites: ${results.numPassedTestSuites} passed, ${results.numFailedTestSuites} failed`,
  )
  console.log(
    `Tests:  ${results.numPassedTests} passed, ${results.numFailedTests} failed`,
  )
  console.log(
    `Time:   ${(results.testResults || []).reduce((s, t) => s + (t.endTime - t.startTime), 0)}ms`,
  )
